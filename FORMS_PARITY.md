...
| **CO** | ✅ `canvas/forms/choosing.rs` — the list **drops** outside the widget; an **editable** combo (`/Ff` 19) is a live text box with a drop button instead, `choosing/typing.rs` | ✅ | ✅ Up/Down move the highlight and write nothing, Enter or Space picks, Escape closes the list on the first press and gives up the ring on the second. On an editable combo the vertical arrows belong to the caret and **Alt+Down** or **F4** opens the list, which is what the product class does | ✅ |
