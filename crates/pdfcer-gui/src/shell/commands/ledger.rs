...
/// The register, built exactly as the application builds it.
///
...
fn registration_succeeds_and_registers_every_command() {
    ...
    // interchange format for stamps, because a stamp collection simply IS an
    // ordinary PDF — one file per category, one page per stamp, the category in
    // `/Info` `/Title` and the names in the catalog's `/Names` → `/Pages` tree.
    //
    // ⇒ So the counter moves by ONE and not by two. Handing someone the file is
...
fn the_icon_coverage_split_adds_up_to_the_registry() {
    ...
    // the operator has been taught inside this application that the mark means
    // *a stamp*, and handing the one command with the word **stamp** in its
    // label a generic export tray would ask him to unlearn it there first.
    //
