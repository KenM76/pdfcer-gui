...
//!
//! Every test below is byte-identical to what it was, de-indented by one level,
//! so a failure here reads exactly as it did before the split. `use super::*;`
//! still resolves to `shell::commands`, and so does every `super::super::` path
//! inside — a `mod tests { … }` and a `mod tests;` have the same parent, which
//! is why this cut is safe to make mechanically.
//!
//! ⚠ The ledgers stay with the assertions rather than moving to the
//! registrations: the running commentary on the command count, the
//! icon-coverage split and the handler-token blocks is written *at the literal
//! it explains*, and that literal is an assertion. Splitting a number from its
//! argument is exactly the drift those ledgers exist to record.
//!
...
fn registration_succeeds_and_registers_every_command() {
    ...
    // is the unit this shell can bind, place, put in a menu and withhold by
    // mode — and `MarkupStyle`'s own doc requires that each control raise
    // **one field**, because a colour picker that also restated the current
    ...
    // mark, and lost whatever they were carrying every time.
    //
    // ★★ A sibling command rather than an extension of
    // `edit.paste_duplicate`, and that WAS checked first: that id already
    // routes by selection kind, and its markup route is *the ordinary
    // paste* (`app::dispatch::clipboard`'s header). Teaching it to
    // duplicate the selection would give one id two unrelated behaviours
    // behind a chord named for the one it would stop having — the same
    ...
    // registry with **no ribbon control at all** — both are in
    // `manifest::TAB_SCOPED`, because their operand is *the corner the pointer
...
fn the_icon_coverage_split_adds_up_to_the_registry() {
    ...
    assert_eq!(
        ...
        // work.
        //
        // Two colour swatches, two drag fields and a combo box are drawn by
        // `Ui::color_edit_button_srgb`, `egui::DragValue` and
        // `egui::ComboBox`. None of those widgets has an icon slot at all,
        // which is the same ground the three Font controls stand on and is
        ...
        // `icons::catalog::mapping`, and the two near-misses were refused on the
        // wrong-picture test rather than on supply: `chevron-up` /
        // `chevron-down` already mean *move this up the PAGE LIST*
        // (`pages.move_up`), so borrowing them would tell an operator that Bring
        // forward reorders the document; `show-points` and `edit-objects` depict
        // a shape's anatomy, not its depth.
        //
        // ⇒ So this is the ledger's ordinary shape — **a refusal with an expiry
        // date**, discharged the day somebody draws four small stacked-rectangle
        // glyphs, and honest until then: four labelled controls in a captioned
