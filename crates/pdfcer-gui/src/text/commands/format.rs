...
//!
//! Split out of [`super`] on 2026-09-14 under R2. That file had reached
//! **1,490 lines** — ten from the ceiling — and the next command to be
//! written was a Format one, so the seam was drawn where the work was
//! already standing rather than at an arbitrary line count.
//!
//! ★ **The seam is a real one, not a line-count convenience.** Every entry
//! here is copy for a control on the **contextual Format tab** or on the
//! canvas context menu that shadows it — the tab `RIBBON_IA.md` §5.8
...
//! each other. The three `view::*` and two `file::*` splits before it were
//! drawn on the same rule.
//!
//! Re-exported by [`super`] with `pub use format::*`, so **nothing changed
//! for a caller**: every call site still writes
//! `crate::text::commands::format_delete()`. That is the same contract the
//! [`super::view`] and [`super::file`] splits hold, and it is what makes a
//! split under R2 a move rather than an API change.
...
/// block of text *"and have the ability to delete them"*. The delete shipped —
/// one line of a text object is a *run*, and `EditSession::delete_text_run`
/// removes one. But it is only reachable once the **Part rung** is entered, and
/// on 2026-09-14 that rung had exactly one entrance: arm the Points tool with
/// the `A` chord, *then* click. Nothing on any surface said so. A capability
/// nobody was told about has not shipped, so this label and this tooltip are
/// the feature, as much as the verb behind them is.
///
...
///
/// # ★★★ What this tooltip does NOT say, and why that is deliberate
///
/// It does not mention **moving**, which is the half of O188 the operator asked
/// for first. `pdfcer-core` has no `move_text_run` — request `G017`, and the
/// gap is real: every other part kind in the crate has both halves, and text
/// has the delete and not the move. A drag at this rung declines with its own
/// worded sentence.
///
/// ⇒ So this tooltip promises only what it can do. Naming the move here would
/// put the disappointment *before* the gesture rather than after it, and
/// `HOW_IT_SHOULD_WORK.md`'s rule is the opposite: a surface states the remedy
/// it HAS. The day `move_text_run` lands, this clause is where it goes.
#[must_use]
