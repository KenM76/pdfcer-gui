...
| **d** | Drag a panel between compartments | **Partly built.** A tab can be dragged **along its own strip**: `dock::drag` holds the gesture, `dock::geometry` answers where every compartment was drawn, and an insertion caret marks the boundary the release will use. It cannot yet be dragged into a *different* stack — a drag over a strip it did not begin in proposes nothing and draws nothing, because there is no drop grammar behind such a release yet. The structure was never the obstacle: one `DockState` owns both sides, so a drag begun on the left is readable on the right with no tree spanning them. `DESIGNS.md` carries the build order. |
