        ...
        (Some("true"), Some("1"), Some("false")) => {
            report.note(
                "★★★ and it landed by SPANNING, with the pin kept: `edit-text-pin` reports \
                 ...
                 several show operators and the span search was told to start AT the pin. On a \
                 page holding the same words twice the same code edits the one that was \
                 clicked rather than the first one on the page",
            );
        }
        ...
        (Some(p), s, o) => {
            return Ok(Some(format!(
                "★★★ THE EDIT LANDED AND THE PLAN'S OWN LINE DOES NOT ADD UP: `pinned={p} \
                 span_from_pin={} one_operator={}`.\n\
                 ★ `canvas::textedit::plan` emits exactly two shapes. \
                 `span_from_pin=0 one_operator=true` — one show operator, the pin alone names \
                 it. `span_from_pin=1 one_operator=false` — several, and the match spans from \
                 the pin. Anything else means the emitter and the decision have drifted apart, \
                 or a field changed spelling. In both cases this check is reading something \
                 other than the decision it was written to judge, and its green on other runs \
                 means less than it looks. Trace: {}.",
                s.unwrap_or("— absent"),
                o.unwrap_or("— absent"),
                session.trace_path().display()
            )));
