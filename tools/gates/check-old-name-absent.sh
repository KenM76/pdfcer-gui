...
# uses to justify a claim, and a record edited to satisfy a naming rule is not a
# record. The specific trigger: `fixtures/subset-font-floor.pdf` embeds a font
# whose name inside the PDF's own bytes carries the old prefix, so every trace
# of the font-substitution check reproduces it — 39 lines in one file, none of
...
# red after it, with nothing having changed about the content. A gate run as a
...
RAW=$(git grep --untracked -nIP "$PATTERN" -- . ":(exclude)evidence/" 2>/dev/null)
STATUS=$?
if [[ "$STATUS" -gt 1 ]]; then
...
EXEMPT_FILES=$(git grep --untracked -lF 'old-name-exempt-file:' -- . 2>/dev/null || true)
