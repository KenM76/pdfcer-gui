# pdfcer-gui — the engineering record

**This was `README.md` until 2026-09-12.** It moved here, unchanged below the
next paragraph, when the front page was rewritten for people who want to *use*
pdfcer rather than build it. Nothing was deleted — including the retractions,
which are the most useful paragraphs on the page and the reason a move was
right rather than a trim.

| Looking for | Read |
|---|---|
| what pdfcer does, and a download | [`README.md`](README.md) |
...
**Measure** — ce dimensions on a scaled drawing, with the calibrated scale
written into pdfcer's own `/PieceInfo` sidecar and read back on reopen. ⚠ **No
`/Measure` or `/Viewport` reader exists** — a PDF that states a scale by standard
means is not read, and the operator calibrates.

**Edit** — text editing and reflow **as an explicit command** (live re-layout
while typing is blocked on the engine and is recorded as never becoming
automatic), vector node editing, object selection with
...
signature reading — with trust evaluation **opt-in and off by default**, so out
of the box every signature reports `NotChecked` — passwords and permissions.

**Out** — print with a live preview beside the settings (the **pop-out window is
`⬜` and has never been rendered**); export to DXF, PNG, JPEG, SVG, EMF, plain
text and form data — ⚠ **there is no export-to-PDF command**, PDF out is
`file.save_a_copy` / `file.save_compacted`; copy page content to the clipboard as
vector, **verified into Word only** — LibreOffice before 25.2 has no route to a
foreign SVG clipboard entry so it takes the EMF, which carries no per-primitive
alpha, and Inkscape is verified nowhere.
...
| **`DOC_DRIFT.md`** | What is wrong in this project's **own documentation**, dated against the commit that invalidated it. R5 makes the docs the logic, so a stale doc comment is a defect — and it is the one class of defect no compiler, test or gate currently catches. Carries the audit's negative control and an honest statement of what was not scanned. |
| **`SWEEP_REPAIRS.md`** | What is wrong in the **`ui-verify` harness**, earned from driven sweeps. Exists because a driven failure is a claim about two programs, and in this project's measured experience the check is wrong more often than the application — so a harness defect reported as an application defect is the expensive mistake. |
| **`DESIGNS.md`** | Designs that are **argued and not yet built** — one section per open operator request, each with what was measured, the design, the alternatives rejected and why, and the driven check it owes. Exists because the expensive half of this work is never the code; it is the measurement and the list of plausible approaches that are wrong. A section is deleted when its design has been built **and driven**, not when the code lands. |
