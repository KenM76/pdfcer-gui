...
| Load a signing identity from a PKCS#12 file (`.pfx`/`.p12`) … | **`unknown` — and the module `pdfcer_core::sign` (101 public items) with it.** Arrived in the engine bump of 2026-09-05 (`03f6004`), as **one subsystem in three `FEATURES.md` rows**: `Pass 10.7` reads the key, `10.8` builds the CAdES `SignedData`, `10.9` signs the document. This shell has wired **verification and trust** and has never authored a signature. ★ The verdict is an honest `unknown` rather than a guessed `wanted`, and the reason is that the missing input is the **operator's**, not this file's: signing is a legal act with a key he owns, and every question that decides the surface — which key store, whether a signature is visible, what `/Reason` and `/Location` say, whether a certifying `/DocMDP` signature is ever offered — is his to answer and none of them has been asked. What is NOT unknown is that it should be raised with him; `OPERATOR_REQUESTS.md` is the place, and this row is deliberately the only thing standing between the subsystem and silence. ⚠ Two limits worth carrying into that conversation: the engine refuses to sign an **encrypted** document outright (its incremental writer cannot append to an encrypted base), and it refuses one with a **pending redaction** — both of which this shell can now produce, so the refusals are reachable rather than theoretical. |
| Build a CAdES `SignedData` (PAdES B-B) … | **`unknown` — same subsystem, same conversation.** See the PKCS#12 row above: `sign::cms_build` is the middle of the three passes and has no surface of its own. It is listed separately because `FEATURES.md` lists it separately, and a register that folded three rows into one would go quiet the day two of them moved apart. |
| Sign a document (APPROVAL signature, PAdES B-B) … | **`unknown` — same subsystem, same conversation.** `EditSession::sign` is the verb a shell would actually call; the other two rows are what it needs. ★ The engine's own note names what is **not** built even at the engine — certifying signatures, signing into a pre-placed field, a password prompt — so a surface built here would be scoped to approval signatures on a new field, which is worth knowing before the design conversation rather than after it. |
| `pdfcer_core::sign` — the whole signing subsystem, 101 public items | **`unknown`, and this row exists to name the MODULE.** `check-engine-api-drift` reports a new module as **one act of attention rather than 101**, on the argument that a subsystem is exactly the thing that gets shipped, noted in a reply nobody reads, and forgotten — which is what happened to PNG/JPEG/SVG export for a day. The three `FEATURES.md` rows above are its three surfaces; this line accounts for the types underneath them (`SignError`, `SignatureAlgorithm`, `sign::apply::SignApplyError`, `sign::cms_build::SubFilter`, `sign::pkcs12::Pkcs12Error` and 95 more). Nothing here holds any of them. |
| `pdfcer_core::edit::CommandKind::AddSignatureField` | **`unknown` — a consequence of the row above, listed because the drift gate lists it.** It is the undo entry `EditSession::sign` pushes. It becomes reachable the day a signing surface exists here and not before, and this shell reads `CommandKind` nowhere. |
