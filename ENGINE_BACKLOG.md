...
## `wanted` — a real gap — **62 of 162** <!-- counted by tools/walk-engine-backlog.py, 2026-09-10; do not retype -->
...
| Row (`FEATURES.md`, wanted) | Why |
|---|---|
| **A stamp's label size is a property, not a derivative of the box it was drawn in — and the box follows the text, not the reverse** (`Pass 287.0`, `1bbb7c1`, 2026-09-09): `TextAnnotSpec::Stamp` takes `style: StampStyle` and `StampFit` | ◑ **HALF WIRED 2026-09-10 — he can state a size when authoring a stamp; he cannot re-size one on the page.** The dialog gained a Size chooser (`text-annot.stamp-size`), driven by `ui-verify`'s `stamp_size_reaches_the_engine`. ★★★ The default is deliberately the DERIVED size: `StampStyle::default()` is a flat 12 pt, and adopting it silently would have shrunk every stamp on his drawings (a 60 pt box derived ≈25 pt), so `StampSize::FitTheBox` keeps the derived size, paired with `GrowToText` rather than `ClipToBox`. Only `GrowToText` is offered; the shrinking fits could not be disclosed at pin `d4a4e3b`. The engine closed both halves on 2026-09-10 (`Pass 292.0`, `c11c1aa`): `stamp_label_parameters` is `pub`, `TextAnnotStyle` grew `font_size` + `stamp_fit`, `StampLabelFit` reports the fit. `fit` is not recoverable, so that control opens on the default. |
| **A placed stamp's label size can now be read back, and changed by a restyle — not fixed forever at authoring time** (`Pass 292.0`, `c11c1aa`, 2026-09-10): `stamp_label_parameters`, `font_size`/`stamp_fit`, `StampLabelFit`. | **wanted, and the highest-value engine delivery here — for a reason that is not the feature.** ★★★ It fixes a live, silent, data-destroying defect in a verb this shell ships: `set_text_annot_style` read a `/Stamp`'s `label: None` as *no custom label*, so recolouring `APPROVED FOR CONSTRUCTION` saved it as `DRAFT`. The pin bump alone reaches it and waits on no wiring. Then: the size field he asked for twice, read via `markup::Current::read`; `ShrinkToBox`/`ClipToBox` become offerable now `StampLabelFit` reports a shrink or clip off-canvas, and `DaUnreadable` is owed either way. ⚠ `fit` is still unrecoverable — a chooser must open on the default, never imply a stored value. ⚠ Two `TextAnnotStyle` literals in `markup::textannot` spell every field, so the bump fails to compile at both; deliberate: `..Default::default()` would take `font_size: None` silently. |
...
| A font-coverage refusal names a working remedy instead of ending on "choose a font that covers it" (`Pass 274.0`, `3340247`) — the standard-14 faces `format_text --set-font` would accept for the same character, filtered by `std14_faces_reachable` (`Pass 279.0`), carried in `Refusal::message` | ⬜ **`wanted`, and it is a disclosure row: the engine now says which face works and this shell throws the sentence away.** `app::actions::textstyle::refusal_of` maps `FormatError::CoverageFailure(_)` to a `&'static str` catalogue sentence and drops the `Refusal` whose `message` names the remedy. What it takes: a `String`-carrying companion on `Declined`, and the engine's face list appended after our own sentence rather than replacing it — ours says what happened, theirs says what to click. The other half, `preview_font_resources_for`, answers the same question before the keystroke and is equally unconsumed. Filed 2026-09-09, pin `fccd6cd`. |
...
| **Read and author Acrobat-compatible custom stamp collections** — a collection **is** an ordinary PDF, one page per stamp (`Pass 288.0`, `554897e`): `pdfcer_core::stamp_file`, `EditSession::set_named_pages`, CLI `stamp-list` / `stamp-pack` | **✅ Both halves built and driven 2026-09-10; placement `blocked` at the engine.** ★★★ The ask contained a false premise and saying so was the work: there is no import or export to match, because a collection is an ordinary PDF — handing somebody the file *is* the export, `file.open` *is* the import. `panels::docprops::stamps` discloses one (detected by name tree, never title); `dialogs::stamp_collection` and `stamps::write` behind `file.stamp_collection` author one, `stamps::folder` finding Acrobat's own stamps folder at runtime. Rows index by name-tree position, never by page (§7.9.6 sorts lexicographically). ☑ Placement WAS blocked and is not any more — the engine answered the same day with `place_page_artwork` (`Pass 293.0`); see the row below, and do not repeat the blocked sentence. |
| **Place a custom stamp's artwork onto a page, as vector — `place_page_artwork` / `pdfcer place-stamp`** (`Pass 293.0`, `56c5e55`, 2026-09-10) | **`wanted`, and it is the half of O169 the operator reaches for next — O172.** `session.place_page_artwork(&collection.view(), stamp_page, page_index, rect)` imports the collection page as a form XObject behind a `/Stamp` `/AP`, so the stamp stays selectable, movable and deletable and survives zooming. What to wire: the existing stamp gallery gains the operator's own categories, and picking one calls this verb instead of `TextAnnotSpec::Stamp`. ★ Three engine findings the surface must not throw away. (1) **Stretch is §12.5.5, not a pdfcer limit** — `/BBox` maps onto `/Rect` with independent axes, so an off-aspect box distorts *by definition*; show `distorted` and the proportional rectangle off-canvas per R8b rule 4, never route around it. (2) A **dynamic** stamp arrives showing its design-time text (`source_widgets_ignored`), so refuse it with a true sentence or place it with a disclosure — `StampEntry::dynamic` is known before the call. (3) No `/Name` is written; whether Acrobat records one is an open gap, not a defect. |
...
| **Make a run bold (or italic) without choosing between two verbs** — `FormatRequest::set_style` … | **`wanted`, and out of this track's scope — filed rather than judged.** `Pass 179.0` gives one verb a per-axis ladder — a real face already on the page, else the standard-14 sibling of the run's own family, else synthesis — with which rung fired disclosed on `FormatReport::style_ladder`. The Format tab already reaches Bold and Italic via `set_font`, so the capability is not absent; the **ladder's disclosure** is, and rule 4 makes it owed — three different answers to one press that look identical on screen. `O47` is the precedent: standard-14 embedding shipped on, silently deciding a licence. ⚠ Filed 2026-09-06 by the signing track, not its owner. ★★ API named so `check-engine-api-drift` is discharged by a reading, not an exemption: `fontdata::std14_styled`; `text_edit::format::StyleLadder` (`requested`, `rung`, `bound`, `synthesised`, `passed_over`); `StyleRung::{AlreadyStyled, RealFaceOnPage, StandardFourteenSibling, Synthetic}`; `FormatError::SynthesisRefusedByPosture`. |
...
| `pdfcer_core::edit::WidgetEdit::with_border_color` and `pdfcer_core::forms::MkColor::to_array` | ⚠ **`next` — this row exists because two source comments in this shell say the opposite and were true when written.** `canvas/forms.rs` and `canvas/forms/boxes.rs` call `/MK` `/BG` unreachable; **both are now false** — `Widget::background` shipped 2026-09-04, `border_color` and `with_border_color` on 2026-09-07 in `fad0d2d`. ⚠⚠⚠ **The consequence was wrongly stated; corrected 2026-09-08.** This row once called the theme-coloured in-canvas field editor operator-visible damage; it is a deliberate design position argued in `canvas::forms` §3 — the editor is *not* a facsimile, because a substituted font cannot promise the document font's advances — repainting it in the field's colours would re-open `DEFECTS.md` **D2**. ⇒ **The editor is not the consumer; the row stays `wanted` for the Properties panel**, showing a field's colours as readable properties — `MkColor::to_array` reads, `with_border_color` authors. ⚠ `MkColor::Cmyk` is kept raw: a swatch names the ink, not an invented colour. |
...
## `blocked` — waiting on something named — **8 of 162** <!-- counted by tools/walk-engine-backlog.py, 2026-09-10; do not retype -->
...
## `unknown` — no opinion formed yet — **0 of 162** <!-- counted by tools/walk-engine-backlog.py, 2026-09-10; do not retype -->
...
## `declined` — deliberately no surface — **7 of 162** <!-- counted by tools/walk-engine-backlog.py, 2026-09-10; do not retype -->
...
## `shipped` — the engine's `[ ]` is stale — **85 of 162** <!-- counted by tools/walk-engine-backlog.py, 2026-09-10; do not retype -->
