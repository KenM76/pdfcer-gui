...
| **3** | G2: the compass overlay, the pointer-to-`DropTarget` resolution over the retained geometry, and cross-compartment drops previewed by replay. |  | The capability the register calls (d). Step 2 is what a replay preview applies to its clone, so this step has something true to show. |
