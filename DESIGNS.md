...
Part 3 of the row — *save the settings inside the PDF* — **is not built**: he
withdrew it in the same sentence he raised it.

---

### What exists today

- **Live state** `PrintDialog` — `dialogs/print/mod.rs:249`, no derives at all,
  constructed on open and dropped on close. Held as
  `Option<PrintDialog>` on `DialogsState` (`dialogs/mod.rs:332`).
- **Persisted subset** `PrintPrefs` — `app/prefs/printing.rs:143`, thirteen
  fields, `Clone + PartialEq`, hand-rolled key/value text format, stored on the
  app-wide `Prefs` as `print` (`app/prefs/mod.rs:842`).
- **The projection** `PrintDialog::habits()` — `dialogs/print/remembered.rs:52`,
  a struct literal with **no** `..Default::default()`, so a new field is a
  compile error there rather than a silent omission. Keep that property.
- **The only writer** `PrintDialog::remember()` — `remembered.rs:92`, called
  from exactly one place: `mod.rs:982`, inside
  `if std::mem::take(&mut self.commit_requested)`, immediately before the
  spool, and **even if the spool then fails**.
- It early-returns writing nothing when `prefs.print == habits`
  (`remembered.rs:95-96`), and `print-remembered` is therefore **not traced**
  when nothing changed. A driven check must not read the absence of that line
  as a failure to save.
- **No snapshot mechanism exists** in `dialogs/print/`. The crate's prior art is
  `dialogs::settings::Draft` (`dialogs/settings/mod.rs:211`) with its
  `working`/`original` pair and `is_dirty()` at `:363`.

#### The argued comment that this request overturns

`mod.rs:965-981` argues, at length, that saving on close would be wrong:
closing without printing is how the operator says *"not this"*, and persisting
an abandoned configuration would make it the next print's opening state.

★ **That reasoning is sound and the operator has overruled it anyway**, because
in practice he sets up a job, closes to go check something, and comes back to
find the setup gone. **The comment must be rewritten, not merely bypassed.** A
comment left arguing for behaviour the code no longer has is the defect this
project has corrected more often than any other.

---

### The shape

He named three outcomes, and they are genuinely three:

| route | meaning |
|---|---|
| **Print** | remember, spool, close *(unchanged — this is O166)* |
| **Cancel** | restore the settings this window opened with, close |
| **Keep and close** | remember, close |

#### Why Cancel — not "Keep and close" — is what the window chrome means

`dialogs.md` G4, enforced at `dialogs/host.rs:363-368` and `:830-833`: the OS
close button, Escape, and the cancel button are **deliberately
indistinguishable**, so that no route out silently means something different
from the other two.

⇒ **X = Escape = Cancel = revert.** All three routes out keep one meaning and
G4 is intact to the letter.

"Keep and close" is then a **fourth, positively-chosen** route — a labelled
button the operator pressed on purpose, which G4 never contemplated and does
not govern. This is the `OK / Cancel / Apply` triad, which is the convention
for a window that owns persistent state, and it puts the *safe* meaning on the
chrome: the hazard here is inheriting a configuration you abandoned (three
hundred copies, the wrong tray), not losing one you can re-enter in four
clicks. The chrome must never be the route that silently keeps a mess.

#### Labels

Not "Close" and "Cancel" — those are synonyms in every reader's head and the
whole design turns on their difference. Left-to-right in the footer:

```
        Keep and close          Cancel        Print
```

with hover text on each saying plainly what happens to the settings. `Print`
stays the accented default and stays bound to Enter; `Cancel` stays bound to
Escape.

`Host::buttons(ui, accept, cancel)` (`host.rs:1036`) takes exactly two and lays
them right-to-left. The third is drawn in the same `right_to_left` layout after
the pair, which puts it leftmost. That is the correct position: it is the
least-used of the three.

---

### What "revert" has to mean, and why it is more than "do not save"

If `Cancel` merely declined to write, it would be indistinguishable from
today's Close — and the word *revert* would be doing no work. It earns its
place on one reachable path:

> **A spool that fails does not close the dialogue** (`mod.rs:1036`), and
> `remember()` has *already run* by then (`mod.rs:982`, before `commit`). So the
> operator can press Print, have it fail, change settings further, and press
> Cancel — at which point the prefs on disk are the ones the failed press wrote,
> not the ones the window opened with.

So Cancel must **write the snapshot back** when the current prefs differ from
it. Literally what he asked for: *"they revert back to what they were when we
opened the print dialogue"*.

### The build

1. **`PrintDialog` gains one field**: `opened_with: PrintPrefs`, cloned from the
   `&PrintPrefs` handed to `PrintDialog::open` (`mod.rs:536`, from
   `dispatch.rs:452`). `PrintPrefs` is already `Clone`.
2. **`remember()` stays exactly as it is** — it is correct, including the
   no-change early return. Two new call sites, not a new mechanism.
3. **`close_requested` grows a reason**: replace the bare `bool` with
   `Option<Dismissal>` where `Dismissal ∈ { Keep, Revert }`, so the single
   return at `mod.rs:1048` can act on it. `frame.closed` (X / Escape) maps to
   `Revert`.
4. **On `Revert`**: if `prefs.print != self.opened_with`, assign and save
   through the same path `remember()` uses, so there is exactly one writer.
5. **On `Keep`**: `self.remember(prefs)` — the same call the Print press makes.
6. **Document-closed** (`dialogs/mod.rs:1299`) drops the dialogue with no
   `show` call and therefore no save. ⇒ **It must behave like `Revert`**, and
   since nothing was written it already does. Say so in a comment so the next
   reader does not "fix" it.
7. **Rewrite `mod.rs:965-981`** to argue the behaviour the code now has, and
   record that the previous ruling was the operator's to overturn and he did.

### Two gaps in the harness this will need
...
`the_print_window_forgets_what_cancel_undid` — open Print, change two fields
that are not at their shipped defaults, press **Cancel**, reopen, require
`print-open` to report the *original* values; then repeat pressing **Keep and
close** and require the *changed* values. Falsify it against a build where
Cancel and Keep are wired the same way — the check must go red in both
...
*"if it would make it a special setting that only works with our program, then
don't implement it."* — saving print settings inside the PDF would be a private
key in the document catalogue that no other reader honours. **Do not build it,
