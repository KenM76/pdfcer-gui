...
| Gates | `bash tools/gates/run-all.sh` | Three states, not two: `0` pass, `1` fail, `3` a gate was SKIPPED for an absent precondition. A skip is not a pass. **A GATE signals its own skip with `exit 2`, and the word SKIP in its output signals nothing** — the runner classifies by exit code alone, so a branch printing SKIP above an `exit 0` lands in the PASSED column and the SKIP-set diff cannot see it, because the name never enters the skip list. **Tally on the `RESULT:` line, never on a pipeline's exit code** — `run-all.sh \| tail` reports `tail`'s zero. **A background task is the worst host for that**, because the completion notification restates the pipe's status in the harness's own voice, beside the word *completed*, and a bare control (`exit 1` in the background) reports `1` correctly — so there is no wrapper bug to find and the only remaining explanation looks like a defect in `run-all.sh`, which exits `1`/`3`/`0` correctly. The tell is the output FILE: a 95-minute run that left 47 lines had a filter eat the detail and the status with it. Launch it `> "$OUT" 2>&1` and the notification reports the runner's own status. A stable pass-count across an engine pin bump proves nothing about whether the documents survived it; the tally moves only when a gate is added. **Run `cargo fmt --all` before starting one** — `cargo fmt` is the LAST gate, so a formatting slip in hand-written Rust is found forty minutes in and costs the entire run. **And start it `CARGO_BUILD_JOBS=2`**: clippy runs after fmt, `--all-targets` spawns a `clippy-driver` per TARGET, and by then the sweep has spent the session's handles on one `sed` per source file — several drivers then die together with `exit code: 0xc0000142, STATUS_DLL_INIT_FAILED` and no diagnostic at all, which reads as four crates breaking at once. It is process count, not memory, whatever a supervisor's kill reason says; the same command alone finishes green in seconds. **★ And `CARGO_BUILD_JOBS=2` is a mitigation, not a cure** — measured 2026-09-21 on the roster that landed `arm.rs`'s `canvas-tool` trace: started with it, the run still lost `clippy-driver`s to `0xc0000142` and printed `RESULT: FAIL — 1 gate(s) found a violation`, while `cargo clippy --workspace --all-targets -- -D warnings` alone finished green in 19 s on the same tree. ⇒ **a `cargo clippy` FAIL inside a roster run is not a verdict on the tree until clippy has been re-run alone**, and that second run is judged on its own `Finished` / `error:` line. The tell is the message itself: `STATUS_DLL_INIT_FAILED` with no diagnostic is process count, and a real lint prints a lint. A sweep killed before its SUMMARY has no verdict, and hand-summing the PASS lines cannot see a gate that never ran. **Every gate in `tools/gates/` carries `--self-test`, with no exception left** — census: `for f in tools/gates/check-*.sh; do grep -q -- --self-test "$f" \|\| echo "$f"; done` (the one hit has its self-test in a sibling `.py`, registered). The gates that fell outside the old *"a grep over source"* qualifier were the last unproven ones, which is backwards: a gate reading a manifest, a lockfile or a network has inputs that are least often present, so its skip and failure branches are the least exercised on a real run. **A self-test needs arms that demand a GREEN**, not only ones that demand a red — a rule that only ever produces red is a rule whose cost nobody has measured, and it trains everyone to edit the file it points at without reading it |
...
**Built so far:** a View-tab toggle draws the layer in the one view there is —
the raster fades towards paper white underneath and the recognised runs paint
over it in the operator's colour, with the blend and the page's block count on
the status bar. The blend is a View-tab slider, present only while the layer is
on, and the colour is a settings swatch written to the preferences file as
...
Also standing:
OCRcer (`D:\dev\OCRcer\`) is the second recogniser, reached through the
engine's `engine_ocrcer` adapter (the engine vendors OCRcer's newest LOCAL
commit — never GitHub). The model is packaged from
`D:\Dev\OCRcer\model\out\ocrcer.ocrw` (**G042** asks the engine for a
versioned home). `reports_confidence()` is an R8 capability (`ocrs` answers
`false`). The LLM rescoring add-in is **O240**, not yet offered by OCRcer.
...
1. **O218 – O225, his eight-row zoom and text report. Four are built and
   ...
   **Built.** O218 — every ceiling the shell derived was
   ...
   in the frame order rather than at each of eight exits — **and O222 was not a
   repaint fault: the typing had never been committed, so the page was
   correctly drawing a document that did not contain it.** Both exits are
   driven and both were falsified against a build with that statement removed.
   ★ `canvas::tool::arm::select` is the sole writer of the armed tool and now
   traces `canvas-tool armed=… from=…` on a change, which is what made a check
   of the tool-change exit possible: three of the four Navigate tools armed
   invisibly, and an armed canvas is the same screenshot as an un-armed one.
