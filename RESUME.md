...
| What | Command | Value, 2026-09-14 13:05 EDT (re-measured against the working tree the O198 pick fix was committed from, every row by its own command, nothing carried over and nothing incremented) |
|---|---|---|
| Engine pin | `grep -m1 -oE 'pdfcer\?branch=main#[0-9a-f]+' Cargo.lock` | `7378c83` — and **the engine is TWO commits ahead of it**, one of which is real code: `025d703d fix(core): reflow no longer refuses a page the producer split into streams`, which is `request_G015` delivered. ⇒ **This pin is the reason O198 claim 2's fault (B) is still open in this tree, and nothing else.** Bump it, re-measure reflow on his sheet, and expect the refusal to change from *"text was added to this page this session"* to the true `R-INV-4` composite-font one. ⚠ The pin is a BRANCH pin and it has moved mid-session without a `cargo update`; re-read the lock in the same breath as quoting it. ★★★ The question a pin row exists to answer is *may I write the sentence "the engine cannot do X"* — and the safe condition is that **no CODE has landed since the pin**, not that two shas match. Today it has |
| Engine HEAD | `cd /d/Dev/pdfcer && git log --oneline -1 main` | `025d703d` — the G015 fix, landed 2026-09-14 09:49 EDT, **with no `reply_G015` or `done_G015` in the request channel yet**. ⇒ A delivery can reach you as a COMMIT before it reaches you as a reply; read the engine's log, not only its outbox. Its message is the measurement: the guard tested a structural property (does the page have more than one `/Contents` stream) as a proxy for a session fact, its premise expired at the engine's `Pass 257.0`, and with it gone his block refuses as `R-INV-4` instead — *"a true sentence about the font instead of a false one about what he did"* |
| Engine version | `grep -A1 'name = "pdfcer-core"' Cargo.lock` | `0.53.0` |
| Last release | `git fetch --tags origin && gh api repos/KenM76/pdfcer-gui/releases/latest` | `v0.5.0-dev.20260914.3`, cut 2026-09-14 10:56 UTC from commit `dd432f9` on engine `7378c83`, `prerelease=false`, `draft=false`, one asset `pdfcergui-20260914-0700-7378c83-dd432f9.zip` of 23,262,555 bytes — read back from the API rather than inferred from having passed the right flag. ⚠ **This row drifts with every commit, including the one that edits this row.** ⚠⚠ **Fetch first.** `gh release create` tags on the REMOTE, so `git describe` in a tree that has not fetched answers with an older tag — that mistake put *"51 commits unreleased"* in a report where the answer was 21. ⇒ **And never pass `--prerelease`**: GitHub hides a pre-release from `releases/latest`. ★★★ **The order the release is cut in:** commit ⇒ `touch crates/pdfcer-gui/build.rs` ⇒ rebuild ⇒ re-drive `the_title_bar_carries_the_build_time` and `about_reports_the_build` against the **shipped** exe ⇒ off-screen smoke launch ⇒ `package-portable --no-build --no-update` ⇒ push ⇒ `gh release create`. `build.rs` has no `rerun-if-changed` on `.git/HEAD`, so without the `touch` the rebuild is a no-op and ships the pre-commit stamp. ★ `package-portable.py`'s header argues AGAINST publishing a release with nothing visible in it; he has read that argument and overruled it — *"always release the latest version when you are done work."* Do not re-perform the cost-benefit |
| Driven checks | `ui-verify --list \| grep -cE '^  [a-z0-9_]+$'` | **226** — one ahead of the shipped release, and the new one is `font_group_real`, O198's. ⚠ **Rebuild the harness before quoting this row.** It answered **0** on the first attempt this session, which is `refuse_if_self_is_stale` working exactly as designed and not a roster; `--list` has been behind that guard since 2026-09-14. ⚠ **The `$` is load-bearing** — `--list` prints a second small table of `--exe` targets whose rows are also two-space-indented lowercase words, and without the end anchor the count silently picks up `pdfcer-gui` and `pdfcer-legacy`. ★ If a count here is about double what you expect, you counted lines: `--list` prints two per check plus an eight-line header, so a `wc -l` answers `2n + 8` |
| Gates | `bash tools/gates/run-all.sh` | **55 passed, 0 failed, 0 skipped**, exit 0 — **two ahead of the shipped release**, and the two are `check-patch-residue` and its self-test. ⇒ **Count a gate change by diffing `run-all.sh`, never by recalling which gate felt new** — this cell has been wrong twice that way. What `check-patch-residue` is for: this session writes Rust through Python patch scripts, and eight distinct ways have now been found for a payload to reach the file mangled while `fmt`, `clippy` and every other gate stay green — the canonical one is a marker helper that maps a LETTER to a star and then fires in the middle of ordinary words — which is why the helper uses `@`, no English word containing it. ⚠ **Do not quote a specimen of the damage in any tracked file**: the gate cannot tell a specimen from a wound, and it is right not to try, because the file most likely to be patched by the script it guards is the one writing about it. This cell tripped it that way on 2026-09-14. ⚠ **The runner exits 1 on a failure, but `run-all.sh \| tail` reports `tail`'s 0.** Tally on the `RESULT:` line, never on the pipeline's exit code |
| Unit tests | `cargo test --workspace` | **4,304 passing, 0 failed, 56 ignored, 4,360 defined** — eight passing and five ignored ahead of the shipped release, all O198's. ★★ **The three totals only reconcile if you count the DELETION too**, and counting only additions is how this cell's attribution was wrong when it was first written: fourteen `#[test]` attributes were added — 4 in `canvas/input.rs` (the pick re-rank), 3 in `tests/ken_sw41177_probe.rs` and 2 in `tests/ken_sw41177_pick.rs` (the headless probes, all `#[ignore]`d), 2 in `panels/properties/tool.rs`, and 1 each in `app/actions/textstyle/tests.rs`, `canvas/textedit/reflow.rs` and `tools/ui-verify/src/checks/font_group.rs` — and **one was removed**, giving 14 − 1 = +13 defined, of which 5 are ignored, leaving +8 passing. Summed over **26** `test result:` lines, then cross-counted by `cargo test --workspace -- --list \| grep -cE ': test$'` = 4,360, and 4,304 + 56 = 4,360 so the two methods agree. Two methods, because a summed figure nobody cross-checks is how the last count drift got in. ★★★ **For a release the two are wired together rather than compared by eye**: the script that writes `FEATURES.md`'s test paragraph sums the transcript itself, takes the cross-count as an argument, and refuses to write the document at all if they disagree. ★ **The cross-count is also how you know a new test RAN**: three `#[test]` functions once compiled clean, reported nothing and executed zero times, because they were nested inside another function |
...
| Request channel | `ls /d/Dev/FeatureRequests/pdfce_FeatureRequests/open \| wc -l` | **47** — one more than at 03:20, and it is `request_G015_page_edited_this_session_fires_on_a_page_the_producer_authored_with_eight_streams.md`, ours, **still sitting in `open/` with no reply beside it even though the engine committed the fix at 09:49**. ⇒ **Watch for the inverse of the usual warning:** the standing one is *a `reply_*` with no `done_*`*; this is *a delivery with no `reply_*` at all*, and the only place it is visible is the engine's git log. ⚠ **Re-`ls` this folder before quoting it** — it changes faster than any other input to this project, it is in no git repository, and nothing warns you |
...
over **221** checks — there are 225 now, so even that figure is not a
...
the program. ⚠ Quoting *"225 checks"* without that sentence beside it reads as
...
### ★★★ 0. O198 — three of its four claims are FIXED, and the remainder is four steps
...
1. ★★★ **Bump the engine pin and re-measure his reflow.** `request_G015`
   landed upstream at `025d703d` this morning — the *"text was added to this
   page this session"* refusal on a page the PRODUCER split into eight
   `/Contents` streams. `cargo update -p pdfcer-core -p pdfcer-render
   -p pdfcer-print`, then drive reflow on page 0 of his drawing and confirm the
   sentence changes from that false one to the true composite-font one.
   ⚠ **The engine kept `PageEditedThisSession` as a variant it no longer
   produces**, and says so in its own commit message. ⇒ `ReflowRefusal::
   PageAlreadyEdited` and its *"save this file and open it again"* remedy
   become unreachable the moment the pin moves, exactly as `PageSetChanged`
   already is. Document that where the variant is defined, in the same commit
   that moves the pin — a sentence that is kept for a future engine is fine; a
   sentence nobody has noticed is unreachable is the defect this project has
   now hit three times.
2. ★★ **Run the full driven sweep.** The re-rank touches **every pick in the
...
3. **Release.** Standing rule: finishing work is itself the trigger. The
   procedure and its four traps are in the *Last release* row above.
4. **Then the rest of claim 1**, which is `O188` — a title-block run the
