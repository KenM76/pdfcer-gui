...
1. **O198's remainder, which is O188** — a title-block run the exporter wrote as
   one lump is still one lump: he can reach that text and drag a line, not take
   it apart. `git -C /d/Dev/pdfcer log --oneline <pin>..main` for the verb.
2. **O181** — installed fonts in Add Text, and the dead Format ribbon controls.
3. **O189** — bookmarks survive a cross-document page drag.
4. **O183** — the nine-part ce-dimension paragraph, part 7 first.
5. **O178** — multi-window tab dragging.
6. **O182** — white seams between the image tiles of a colour rendering.
7. **O194 steps 1, 4 and 5** — the ~30 surfaces in `UNIT_SURFACES.md` with no
...
8. **O195** — smart select in Review. `smart::enabled` defaults on and
...
9. **O176 — his verdict, not a repair.** At fit zoom a form field is about 28 px
...
10. **Wire `SignReport::appearance_lines`** — the one engine delivery genuinely
...
11. **Run a full driven sweep.** Ninety-five minutes taking the real cursor and
